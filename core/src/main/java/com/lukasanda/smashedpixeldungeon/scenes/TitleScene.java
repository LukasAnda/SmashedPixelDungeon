...
import com.lukasanda.smashedpixeldungeon.effects.Fireball;
import com.lukasanda.smashedpixeldungeon.messages.Messages;
import com.lukasanda.smashedpixeldungeon.ui.Archs;
import com.lukasanda.smashedpixeldungeon.ui.ChangesButton;
import com.lukasanda.smashedpixeldungeon.ui.ExitButton;
import com.lukasanda.smashedpixeldungeon.ui.LanguageButton;
import com.lukasanda.smashedpixeldungeon.ui.PrefsButton;
	...
	public void create() {
		...
		DashboardItem btnBadges = new DashboardItem( Messages.get(this, "badges"), 3 ) {
			...
			protected void onClick() {
				SmashedPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		add(btnBadges);
		...
		DashboardItem btnAbout = new DashboardItem( Messages.get(this, "about"), 1 ) {
			@Override
			protected void onClick() {
				SmashedPixelDungeon.switchNoFade( AboutScene.class );
			}
		};
		add( btnAbout );
		...
		DashboardItem btnPlay = new DashboardItem( Messages.get(this, "play"), 0 ) {
			@Override
			protected void onClick() {
				SmashedPixelDungeon.switchNoFade( StartScene.class );
			}
		};
		add( btnPlay );
		...
		DashboardItem btnRankings = new DashboardItem( Messages.get(this, "rankings"), 2 ) {
			@Override
			protected void onClick() {
				SmashedPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		add( btnRankings );
		...
		if (SPDSettings.landscape()) {
			...
			btnBadges.setPos( w / 4 - btnBadges.width()/2, btnPlay.top() * 1.125f + DashboardItem.SIZE );
			btnAbout.setPos( 3*w / 4 - btnAbout.width()/2, btnBadges.top() );
			btnSoulShop.setPos(w/2-btnSoulShop.width()/2,btnPlay.bottom()-btnSoulShop.height()/4);
		}
