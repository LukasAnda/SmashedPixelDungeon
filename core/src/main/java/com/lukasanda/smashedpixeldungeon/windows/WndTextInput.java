...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		SmashedPixelDungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(SmashedPixelDungeon.instance);
				...
				SmashedPixelDungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			SmashedPixelDungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) SmashedPixelDungeon
					...
					SmashedPixelDungeon.updateSystemUI();
