package com.lukasanda.smashedpixeldungeon.effects;
...
public class AbilityBanner extends Image {
    
    private enum State {
        FADE_IN, STATIC, FADE_OUT
    };
    private AbilityBanner.State state;
    ...
    private static AbilityBanner current;
    
    private AbilityBanner( int index ) {
        super( Assets.ABILITIES );
        ...
        state = AbilityBanner.State.FADE_IN;
        time = FADE_IN_TIME;
        //TODO Need to add proper sound
    ...
    public void update() {
        ...
        if (time >= 0) {
            ...
            switch (state) {
                case FADE_IN:
                    time = STATIC_TIME;
                    state = AbilityBanner.State.STATIC;
                ...
                case STATIC:
                    time = FADE_OUT_TIME;
                    state = AbilityBanner.State.FADE_OUT;
                    break;
                case FADE_OUT:
                    killAndErase();
                    break;
    ...
    public static AbilityBanner show( int image ) {
        if (current != null) {
            current.killAndErase();
        }
        return (current = new AbilityBanner( image ));
    }
    
    public static Image image( int index ) {
        Image image = new Image( Assets.ABILITIES );
        if (atlas == null) {
            atlas = new TextureFilm( image.texture, 32, 32 );
        }
